...
public class ChartMake {
	...
	public static void barChart(HttpServletRequest request, Map<String, Integer> resultMap) {
		...
		try { // 데이터로 사용할 카테고리 데이터 셋을 생정
			...
			CategoryPlot plot = chart.getCategoryPlot(); // X축 라벨
			plot.getDomainAxis().setLabelFont(font); // 성별 //X축 도메인
			plot.getDomainAxis().setTickLabelFont(font); // 남자,여자 //Y축 라벨
			plot.getRangeAxis().setLabelFont(font); // 인원수(단위 : 명) //Y축 범위
